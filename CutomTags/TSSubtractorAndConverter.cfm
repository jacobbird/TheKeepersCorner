...
<cfset TodaysDay = NumberFormat(left(right(now(),13),2),9)>
...
<cfset TodaysMonth = NumberFormat(left(right(now(),16),2),9)>
<cfset TodaysMonthString = ToString(TodaysMonth)>
<cfset TodaysMonthExtendedString = "0"&"#variables.TodaysMonthString#">
</cfif>

<cfset TodaysYear = NumberFormat(left(right(now(),21),4),9999)>
<cfset TodaysHours = NumberFormat(left(right(#variables.TodaysTime#,2),2))>
<cfset TodaysMinutes = NumberFormat(left(right(#variables.TodaysTime#,5),2))>
<cfset TodaysSeconds = NumberFormat(left(right(#variables.TodaysTime#,8),2))>
...
<cfset NumberofDaysinthemonth = 31>
<cfelse>
<cfthrow message = "An unexpected error has occured please contact you site administrator: DateFinder ERROR 001 'Not a Valid Month'">
</cfif>
...
<cfset TodaysValue = "#variables.StartYear##variables.StartMonthExtendedString##variables.StartDayExtendedString#">
